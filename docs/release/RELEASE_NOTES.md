...
최종 컴파일 뒤 Node.js 22/24로 전체 배포 JavaScript의 구문을 검사하고 source checkout의
`./bin/cat --help` 시작 경로를 한 번 확인한다. 그 외 대화형 앱, TUI, provider, web, MCP,
shell/process, worktree, SSH/clipboard, legacy import, installer 또는 standalone bundled runtime은
실행해 검증하지 않았다. artifact 생성 성공도 실환경 설치·운영 준비나 보안 검증 완료를 뜻하지 않는다.
